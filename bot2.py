...
def start_handler(message: Message) -> None:
    bot.send_message(message.chat.id, text=f"🔎 <b>Поиск ИИН</b>\n\n{constants.DATE_REQUEST}")
    bot.register_next_step_handler(message, date_handler)


@bot.message_handler()
def default_handler(message: Message) -> None:
    bot.set_message_reaction(message.chat.id, message.id, reaction=[ReactionTypeEmoji("🤷‍♂️")])
...
def date_handler(message: Message) -> None:
    if message.content_type == "text":
        try:
            ...
            bot.set_message_reaction(message.chat.id, message.id, reaction=[ReactionTypeEmoji("👎")])
